...
description: >
  ...
  Args:
      inputs: {
            ...
            `req_analysis`: 需求分析详情 调调用 issue-analyst 工作流获取【强制】)
      }
---
...
## MeterSphere 落库字段转换（ms_cases convert / ms_design 自动处理）

生成的 CASE 节点不需要关心 MeterSphere 字段，落库时按以下映射自动转换：

| 生成器字段（instance）        | MeterSphere 落库字段                          |
| ----------------------------- | --------------------------------------------- |
| `preconditions`               | `prerequisite`                                |
| `priority` (P0-P3)            | 模板 `functional_priority` customField        |
| `steps[].action`              | `steps[].desc`（步骤描述）                    |
| `steps[].expected`            | `steps[].result`（预期结果）                  |
| `kind=CASE` + `name`          | 用例名称；TEST_POINT 父节点会加 `{测试点}::` 前缀 |
| FEATURE/SCENE 祖先节点        | `tags`=[FEATURE 名, SCENE 名] + 模块归属      |

因此生成器**只须保证**：`instance.preconditions`、`instance.priority`（P0/P1/P2/P3）、`instance.steps` 每项含 `action` 与 `expected`，且 `name` 不含测试点前缀。
